...
def build_dataframe(body):
    ...
    #df["biomass"] = df["biomass"].apply(lambda x: x*WASTE*(10**6))
    #df["fossil_gas"] = df["fossil_gas"].apply(lambda x: x*GAS*(10**6))
    #df["fossil_hard_coal"] = df["fossil_hard_coal"].apply(lambda x: x*COAL*(10**6))
    #df["fossil_oil"] = df["fossil_oil"].apply(lambda x: x*OIL*(10**6))
    #df["nuclear"] = df["nuclear"].apply(lambda x: x*NUCLEAR*(10**6))
    #df["solar"] = df["solar"].apply(lambda x: x*SOLAR*(10**6))
    #df["waste"] = df["waste"].apply(lambda x: x*WASTE*(10**6))
    #df["wind_onshore"] = df["wind_onshore"].apply(lambda x: x*WIND*(10**6))
